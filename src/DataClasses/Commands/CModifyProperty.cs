...
namespace Editor.DataClasses.Commands
{
   ...
   public class CModifyPropertyMultiple<T> : SaveableCommandBasic
   {
      private List<T> _newValue;
      private readonly List<Saveable> _targets;
      private readonly List<T> _oldValue;
      private readonly PropertyInfo _propInfo;

      public CModifyPropertyMultiple(PropertyInfo property, List<Saveable> targets, List<T> values, out bool change, bool executeOnInit = true)
      {
         _propInfo = property;
         _targets = targets;
         _newValue = values;
         ...
         for (var i = _targets.Count - 1; i >= 0; i--)
         {
            if (_targets[i].GetProperty<T>(property)!.Equals(values[i]))
            {
               _targets.RemoveAt(i);
               _newValue.RemoveAt(i);
            }
            else
               _oldValue.Insert(0, (T)_propInfo?.GetValue(_targets[i])!);
         }
      ...
      private void InternalExecute()
      {
         Saveable.SetFieldMultipleDifferentSilent(_targets, _newValue, _propInfo);
      }
