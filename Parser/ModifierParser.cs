...
namespace Editor.Parser
{
   public static class ModifierParser
   {
      ...
      {
         Globals.ModifierKeys = new string[CountryModifiers.Count + ProvinceModifiers.Count + EstateModifiers.Count + GovernmentMechanics.Count + FactionModifiers.Count];


         var cnt = 0;
         foreach (var cmod in CountryModifiers)
         {
            Globals.ModifierKeys[cnt] = cmod;
            CountryModifierToIndex[cmod] = cnt++;
         }

         foreach (var pmod in ProvinceModifiers)
         {
      ...
      {
         return CustomModifierTrigger.Contains(value);
      }
